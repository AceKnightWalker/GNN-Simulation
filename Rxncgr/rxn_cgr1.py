...
class RxnGraph:
    def __init__(self, smiles, extra=False):
        ...
        # Compute centralities only if extra is True, else empty dict
        centralities = compute_centralities(mol_reac) if extra else {}
        ...
        for a1 in range(n_atoms):
            ...
            # If extra is True, add centralities, else add empty list
            if extra:
                centrality_feats = [
                    centralities['degree'].get(a1, 0.0),
                    centralities['closeness'].get(a1, 0.0),
                    centralities['betweenness'].get(a1, 0.0)
                ]
            else:
                centrality_feats = []

            # Final feature vector
            f_atom = f_atom_reac + f_atom_diff + centrality_feats
            self.f_atoms.append(f_atom)
...
class ChemDataset(Dataset):
    def __init__(self, smiles, labels, mode='rxn', transform=None, extra = False):
    ...
    def process_key(self, key):
        smi = self.smiles[key]
        if self.mode == 'rxn':
            molgraph = RxnGraph(smi, extra=self.extra)
        else:
...
def construct_loader(data_path, shuffle=True, batch_size=75, mode='rxn', transform=None, extra=False):
    data_df = pd.read_csv(data_path)
    smiles = data_df.iloc[:, 0].values
    labels = data_df.iloc[:, 1].values.astype(np.float32)
    dataset = ChemDataset(smiles, labels, mode, transform=transform, extra=extra)
